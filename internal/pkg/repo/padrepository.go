...
func (r *PadRepository) Create(pad *entities.Pad) (*entities.Pad, error) {
	if err := r.store.Db().QueryRow(
		"INSERT INTO \"pad\" (name, description, website_link, price, target_id, owner_id) VALUES($1, $2, $3, $4, $5, $6) RETURNING id;",
		pad.Name, pad.Description, pad.Website_link, pad.Price, pad.Target_id, pad.Owner_id,
	).Scan(&pad.Id); err != nil {
		return nil, err
	}
...
func (r *PadRepository) Update(pad *entities.Pad) error {
	_, err := r.store.Db().Exec(
		"UPDATE \"pad\" SET name=$1, description=$2, website_link=$3, price=$4, target_id=$5, owner_id=$6 WHERE id=$7;",
		pad.Name, pad.Description, pad.Website_link, pad.Price, pad.Target_id, pad.Owner_id, pad.Id,
	)
	if err != nil {
		return err
	}
...
func (r *PadRepository) Read(id int) ([]*entities.Pad, error) {
	...
	for rows.Next() {
		pad := &entities.Pad{}
		err := rows.Scan(&pad.Id, &pad.Name, &pad.Description, &pad.Website_link, &pad.Price, &pad.Target_id, &pad.Owner_id)
		if err != nil {
			return nil, err
		}
...
func (r *PadRepository) getList(id int) (*sql.Rows, error) {
	return r.store.Db().Query("SELECT id, name, description, website_link, price, target_id, owner_id FROM \"pad\" WHERE owner_id=$1;", id)
}

func (r *PadRepository) getOne(id int) (*sql.Rows, error) {
	return r.store.Db().Query("SELECT id, name, description, website_link, price, target_id, owner_id FROM \"pad\" WHERE id=$1;", id)
}
